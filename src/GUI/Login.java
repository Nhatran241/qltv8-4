...
public class Login extends javax.swing.JFrame {
    ...
    private void btLoginActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btLoginActionPerformed
        ...
        int status =nhanvienBUS.kiemtradangnhap(txtTaiKhoan.getText(),txtMatKhau.getText());
        System.err.println(status+"");
        if(status!=-1){
            new FrameMain(status).setVisible(true);
            this.dispose();
        }else{
            JOptionPane.showMessageDialog(null, "Bạn đã nhập sai mã nhân viên hoặc mật khẩu", "Thông báo", 1);
        }
